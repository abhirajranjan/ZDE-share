...
void udp::init()
{
    udpfd_recv = networking::generate_socket(AF_INET, SOCK_DGRAM, 0);
    udpfd_sender = networking::generate_socket(AF_INET, SOCK_DGRAM, 0);

    networking::setup_addr(AF_INET, PORT, &udpaddr_recv);
    networking::setup_addr(AF_INET,  PORT,  &MULTICAST_IP, &udpaddr_sender);

    networking::bind_socket(udpfd_recv, (struct sockaddr*) &udpaddr_recv, sizeof(udpaddr_recv));
    ...
    if (setsockopt(udpfd_recv, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*) &mreq, sizeof(mreq)) < 0)
    {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
