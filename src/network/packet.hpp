...
namespace packet{
    int generate_indentity_packet(std::string nickname, std::string tcpIp, int tcpPort, std::string os, json* out){
        pckt_t pck = identity;
        (*out)["pckt"] = pck;
        (*out)["name"] = nickname;
        (*out)["ip"] = tcpIp;
    ...
    int validate_packet(json pkt_j){
    ...
    int parse_packet(std::string packet){
